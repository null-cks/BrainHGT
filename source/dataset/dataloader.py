...
def init_dataloader(cfg: DictConfig,
    ...
    dataset = utils.TensorDataset(
        ...
        adj[:train_length+val_length+test_length],
...
def init_stratified_dataloader(cfg: DictConfig,
    ...
    for train_index, test_valid_index in split.split(final_pearson, stratified):
        final_timeseires_train, final_pearson_train, adj_train, labels_train = final_timeseires[
            train_index], final_pearson[train_index], adj[train_index], labels[train_index]
        final_timeseires_val_test, final_pearson_val_test, adj_val_test, labels_val_test = final_timeseires[
            test_valid_index], final_pearson[test_valid_index], adj[test_valid_index], labels[test_valid_index]
        stratified = stratified[test_valid_index]

    split2 = StratifiedShuffleSplit(n_splits=1, test_size=val_length)
    for test_index, valid_index in split2.split(final_timeseires_val_test, stratified):
        final_timeseires_test, final_pearson_test, adj_test, labels_test = final_timeseires_val_test[
            test_index], final_pearson_val_test[test_index], adj_val_test[test_index], labels_val_test[test_index]
        final_timeseires_val, final_pearson_val, adj_val, labels_val = final_timeseires_val_test[
            valid_index], final_pearson_val_test[valid_index], adj_val_test[valid_index], labels_val_test[valid_index]
    ...
    val_dataset = utils.TensorDataset(
        final_timeseires_val, final_pearson_val, adj_val, labels_val
    )

    test_dataset = utils.TensorDataset(
        final_timeseires_test, final_pearson_test, adj_test, labels_test
    )
